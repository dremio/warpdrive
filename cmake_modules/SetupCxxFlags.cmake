...
if(NOT DEFINED WARPDRIVE_CPU_FLAG)
  if(CMAKE_SYSTEM_PROCESSOR MATCHES "aarch64|ARM64|arm64")
    set(WARPDRIVE_CPU_FLAG "armv8")
  elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "armv7")
    set(WARPDRIVE_CPU_FLAG "armv7")
  elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "powerpc|ppc")
    set(WARPDRIVE_CPU_FLAG "ppc")
  elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "s390x")
    set(WARPDRIVE_CPU_FLAG "s390x")
  else()
    set(WARPDRIVE_CPU_FLAG "x86")
  endif()
endif()

# Check architecture specific compiler flags
if(WARPDRIVE_CPU_FLAG STREQUAL "x86")
  # x86/amd64 compiler flags, msvc/gcc/clang
  if(MSVC)
    set(WARPDRIVE_SSE4_2_FLAG "")
    set(WARPDRIVE_AVX2_FLAG "/arch:AVX2")
    set(WARPDRIVE_AVX512_FLAG "/arch:AVX512")
    set(CXX_SUPPORTS_SSE4_2 TRUE)
  else()
    set(WARPDRIVE_SSE4_2_FLAG "-msse4.2")
    set(WARPDRIVE_AVX2_FLAG "-march=haswell")
    # skylake-avx512 consists of AVX512F,AVX512BW,AVX512VL,AVX512CD,AVX512DQ
    set(WARPDRIVE_AVX512_FLAG "-march=skylake-avx512 -mbmi2")
    # Append the avx2/avx512 subset option also, fix issue WARPDRIVE-9877 for homebrew-cpp
    set(WARPDRIVE_AVX2_FLAG "${WARPDRIVE_AVX2_FLAG} -mavx2")
    set(WARPDRIVE_AVX512_FLAG
        "${WARPDRIVE_AVX512_FLAG} -mavx512f -mavx512cd -mavx512vl -mavx512dq -mavx512bw")
    check_cxx_compiler_flag(${WARPDRIVE_SSE4_2_FLAG} CXX_SUPPORTS_SSE4_2)
  endif()
  check_cxx_compiler_flag(${WARPDRIVE_AVX2_FLAG} CXX_SUPPORTS_AVX2)
  if(MINGW)
    ...
    set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} ${WARPDRIVE_AVX512_FLAG}")
  ...
  # Runtime SIMD level it can get from compiler and WARPDRIVE_RUNTIME_SIMD_LEVEL
  if(CXX_SUPPORTS_SSE4_2 AND WARPDRIVE_RUNTIME_SIMD_LEVEL MATCHES
                             "^(SSE4_2|AVX2|AVX512|MAX)$")
    set(WARPDRIVE_HAVE_RUNTIME_SSE4_2 ON)
    add_definitions(-DWARPDRIVE_HAVE_RUNTIME_SSE4_2)
  endif()
  if(CXX_SUPPORTS_AVX2 AND WARPDRIVE_RUNTIME_SIMD_LEVEL MATCHES "^(AVX2|AVX512|MAX)$")
    set(WARPDRIVE_HAVE_RUNTIME_AVX2 ON)
    add_definitions(-DWARPDRIVE_HAVE_RUNTIME_AVX2 -DWARPDRIVE_HAVE_RUNTIME_BMI2)
  endif()
  if(CXX_SUPPORTS_AVX512 AND WARPDRIVE_RUNTIME_SIMD_LEVEL MATCHES "^(AVX512|MAX)$")
    set(WARPDRIVE_HAVE_RUNTIME_AVX512 ON)
    add_definitions(-DWARPDRIVE_HAVE_RUNTIME_AVX512 -DWARPDRIVE_HAVE_RUNTIME_BMI2)
  endif()
  if(WARPDRIVE_SIMD_LEVEL STREQUAL "DEFAULT")
    set(WARPDRIVE_SIMD_LEVEL "SSE4_2")
  endif()
elseif(WARPDRIVE_CPU_FLAG STREQUAL "ppc")
  # power compiler flags, gcc/clang only
  set(WARPDRIVE_ALTIVEC_FLAG "-maltivec")
  check_cxx_compiler_flag(${WARPDRIVE_ALTIVEC_FLAG} CXX_SUPPORTS_ALTIVEC)
  if(WARPDRIVE_SIMD_LEVEL STREQUAL "DEFAULT")
    set(WARPDRIVE_SIMD_LEVEL "NONE")
  endif()
elseif(WARPDRIVE_CPU_FLAG STREQUAL "armv8")
  # Arm64 compiler flags, gcc/clang only
  set(WARPDRIVE_ARMV8_ARCH_FLAG "-march=${WARPDRIVE_ARMV8_ARCH}")
  check_cxx_compiler_flag(${WARPDRIVE_ARMV8_ARCH_FLAG} CXX_SUPPORTS_ARMV8_ARCH)
  if(WARPDRIVE_SIMD_LEVEL STREQUAL "DEFAULT")
    set(WARPDRIVE_SIMD_LEVEL "NEON")
  endif()
endif()
...
# WARPDRIVE-6848: Do not use GNU (or other CXX) extensions
...
if(WIN32)
  ...
  if(MSVC)
    if(MSVC_VERSION VERSION_LESS 19)
      ...
      by Warpdrive. Found version ${CMAKE_CXX_COMPILER_VERSION}.")
    endif()

    # WARPDRIVE-1931 See https://github.com/google/googletest/issues/1318
    #
    ...
    if(CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
      ...
      #
      # WARPDRIVE-2986: Without /EHsc we get C4530 warning
    ...
    # cpp/src/warpdrive/compute/kernels/scalar_string_test.cc
...
message(STATUS "Warpdrive build warning level: ${BUILD_WARNING_LEVEL}")

macro(warpdrive_add_werror_if_debug)
...
if("${BUILD_WARNING_LEVEL}" STREQUAL "CHECKIN")
  ...
  elseif(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    ...
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} -Wno-maybe-uninitialized")
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} -Wno-uninitialized")
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} -Wno-unused-function")
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} -Wno-unused-variable")
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} -Wno-write-strings")
...
if(WARPDRIVE_CPU_FLAG STREQUAL "x86")
  ...
  if(WARPDRIVE_SIMD_LEVEL STREQUAL "AVX512")
    if(NOT CXX_SUPPORTS_AVX512)
      message(FATAL_ERROR "AVX512 required but compiler doesn't support it.")
    endif()
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} ${WARPDRIVE_AVX512_FLAG}")
    add_definitions(-DWARPDRIVE_HAVE_AVX512 -DWARPDRIVE_HAVE_AVX2 -DWARPDRIVE_HAVE_BMI2
                    -DWARPDRIVE_HAVE_SSE4_2)
  elseif(WARPDRIVE_SIMD_LEVEL STREQUAL "AVX2")
    if(NOT CXX_SUPPORTS_AVX2)
      message(FATAL_ERROR "AVX2 required but compiler doesn't support it.")
    endif()
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} ${WARPDRIVE_AVX2_FLAG}")
    add_definitions(-DWARPDRIVE_HAVE_AVX2 -DWARPDRIVE_HAVE_BMI2 -DWARPDRIVE_HAVE_SSE4_2)
  elseif(WARPDRIVE_SIMD_LEVEL STREQUAL "SSE4_2")
    if(NOT CXX_SUPPORTS_SSE4_2)
      message(FATAL_ERROR "SSE4.2 required but compiler doesn't support it.")
    endif()
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} ${WARPDRIVE_SSE4_2_FLAG}")
    add_definitions(-DWARPDRIVE_HAVE_SSE4_2)
  elseif(NOT WARPDRIVE_SIMD_LEVEL STREQUAL "NONE")
    message(WARNING "WARPDRIVE_SIMD_LEVEL=${WARPDRIVE_SIMD_LEVEL} not supported by x86.")
  endif()
endif()

if(WARPDRIVE_CPU_FLAG STREQUAL "ppc")
  if(CXX_SUPPORTS_ALTIVEC AND WARPDRIVE_ALTIVEC)
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} ${WARPDRIVE_ALTIVEC_FLAG}")
  endif()
endif()

if(WARPDRIVE_CPU_FLAG STREQUAL "armv8")
  if(WARPDRIVE_SIMD_LEVEL STREQUAL "NEON")
    set(WARPDRIVE_HAVE_NEON ON)

    if(NOT CXX_SUPPORTS_ARMV8_ARCH)
      message(FATAL_ERROR "Unsupported arch flag: ${WARPDRIVE_ARMV8_ARCH_FLAG}.")
    endif()
    if(WARPDRIVE_ARMV8_ARCH_FLAG MATCHES "native")
      message(FATAL_ERROR "native arch not allowed, please specify arch explicitly.")
    endif()
    set(CXX_COMMON_FLAGS "${CXX_COMMON_FLAGS} ${WARPDRIVE_ARMV8_ARCH_FLAG}")

    add_definitions(-DWARPDRIVE_HAVE_NEON)
    ...
    if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU" AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS
      ...
      if(WARPDRIVE_ARMV8_ARCH_FLAG MATCHES "\\+crypto")
        add_definitions(-DWARPDRIVE_HAVE_ARMV8_CRYPTO)
      endif()
      # armv8.1+ implies crc support
      if(WARPDRIVE_ARMV8_ARCH_FLAG MATCHES "armv8\\.[1-9]|\\+crc")
        add_definitions(-DWARPDRIVE_HAVE_ARMV8_CRC)
      endif()
    endif()
  elseif(NOT WARPDRIVE_SIMD_LEVEL STREQUAL "NONE")
    message(WARNING "WARPDRIVE_SIMD_LEVEL=${WARPDRIVE_SIMD_LEVEL} not supported by Arm.")
  endif()
endif()
...
if(NOT WIN32 AND NOT APPLE)
  ...
  if(GOLD_VERSION)
    ...
    if("${GOLD_VERSION}" VERSION_LESS "1.12")
      set(WARPDRIVE_BUGGY_GOLD 1)
    endif()
    if(MUST_USE_GOLD)
      message("Using hard-wired gold linker (version ${GOLD_VERSION})")
      if(WARPDRIVE_BUGGY_GOLD)
        if("${WARPDRIVE_LINK}" STREQUAL "d" AND "${CMAKE_BUILD_TYPE}" STREQUAL "RELEASE")
          message(SEND_ERROR "Configured to use buggy gold with dynamic linking "
                             "in a RELEASE build")
        endif()
      endif()
    elseif(NOT WARPDRIVE_BUGGY_GOLD)
...
#   -DWARPDRIVE_CXXFLAGS="-g" to add them
if(NOT MSVC)
  if(WARPDRIVE_GGDB_DEBUG)
    set(WARPDRIVE_DEBUG_SYMBOL_TYPE "gdb")
    set(C_FLAGS_DEBUG "-g${WARPDRIVE_DEBUG_SYMBOL_TYPE} -O0")
    set(C_FLAGS_FASTDEBUG "-g${WARPDRIVE_DEBUG_SYMBOL_TYPE} -O1")
    set(CXX_FLAGS_DEBUG "-g${WARPDRIVE_DEBUG_SYMBOL_TYPE} -O0")
    set(CXX_FLAGS_FASTDEBUG "-g${WARPDRIVE_DEBUG_SYMBOL_TYPE} -O1")
  else()
...
if(MSVC)
  ...
  if(NOT WARPDRIVE_USE_STATIC_CRT)
