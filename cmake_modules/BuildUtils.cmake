...
set(WARPDRIVE_LIBRARY_PATH_SUFFIXES
...
set(WARPDRIVE_BOOST_PROCESS_COMPILE_DEFINITIONS)
if(WIN32 AND CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
  ...
  #                              ${WARPDRIVE_BOOST_PROCESS_COMPILE_DEFINITIONS})
  list(APPEND WARPDRIVE_BOOST_PROCESS_COMPILE_DEFINITIONS "BOOST_USE_WINDOWS_H=1")
endif()
...
function(REUSE_PRECOMPILED_HEADER_LIB TARGET_NAME LIB_NAME)
  if(WARPDRIVE_USE_PRECOMPILED_HEADERS)
...
function(ADD_WARPDRIVE_LIB LIB_NAME)
  set(options)
  set(one_value_args
      BUILD_SHARED
      BUILD_STATIC
  ...
  # Allow overriding WARPDRIVE_BUILD_SHARED and WARPDRIVE_BUILD_STATIC
  if(DEFINED ARG_BUILD_SHARED)
    set(BUILD_SHARED ${ARG_BUILD_SHARED})
  else()
    set(BUILD_SHARED ${WARPDRIVE_BUILD_SHARED})
  endif()
  if(DEFINED ARG_BUILD_STATIC)
    set(BUILD_STATIC ${ARG_BUILD_STATIC})
  else()
    set(BUILD_STATIC ${WARPDRIVE_BUILD_STATIC})
  endif()
  if(ARG_OUTPUT_PATH)
  ...
  if(USE_OBJLIB)
    ...
    if(ARG_PRECOMPILED_HEADERS AND WARPDRIVE_USE_PRECOMPILED_HEADERS)
  ...
  if(BUILD_SHARED)
    ...
    set_target_properties(${LIB_NAME}_shared
                          PROPERTIES LIBRARY_OUTPUT_DIRECTORY "${OUTPUT_PATH}"
                                     ...
                                     VERSION "${WARPDRIVE_FULL_SO_VERSION}"
                                     SOVERSION "${WARPDRIVE_SO_VERSION}")
    ...
    if (DEFINED ARG_FORCE_LOAD_LIB)
      if (APPLE)
        ...
        endforeach(FORCED_LOAD_LIB ${ARG_FORCE_LOAD_LIB})
    ...
    if(APPLE)
      if(WARPDRIVE_INSTALL_NAME_RPATH)
  ...
  if(BUILD_STATIC)
    ...
    if(WARPDRIVE_BUILD_STATIC AND WIN32)
      target_compile_definitions(${LIB_NAME}_static PUBLIC WARPDRIVE_STATIC)
    endif()
  ...
  if(ARG_CMAKE_PACKAGE_NAME)
    warpdrive_install_cmake_find_module("${ARG_CMAKE_PACKAGE_NAME}")
    ...
    configure_package_config_file("${CONFIG_CMAKE}.in" "${BUILT_CONFIG_CMAKE}"
                                  INSTALL_DESTINATION "${WARPDRIVE_CMAKE_INSTALL_DIR}")
    install(FILES "${BUILT_CONFIG_CMAKE}" DESTINATION "${WARPDRIVE_CMAKE_INSTALL_DIR}")
  ...
  if(ARG_PKG_CONFIG_NAME)
    warpdrive_add_pkg_config("${ARG_PKG_CONFIG_NAME}")
  endif()
...
# example, if you have src/warpdrive/foo/bar-benchmark.cc, then PREFIX "foo" will
...
function(ADD_BENCHMARK REL_BENCHMARK_NAME)
  ...
  if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${REL_BENCHMARK_NAME}.cc)
    ...
    if(ARG_STATIC_LINK_LIBS)
      ...
      target_link_libraries(${BENCHMARK_NAME} PRIVATE ${WARPDRIVE_BENCHMARK_LINK_LIBS})
    endif()
...
# Arguments after the test name will be passed to set_tests_properties().
#
# \arg ENABLED if passed, add this unit test even if WARPDRIVE_BUILD_TESTS is off
# \arg PREFIX a string to append to the name of the test executable. For
# example, if you have src/warpdrive/foo/bar-test.cc, then PREFIX "foo" will create
...
function(ADD_TEST_CASE REL_TEST_NAME)
  ...
  if(ARG_STATIC_LINK_LIBS)
    ...
    target_link_libraries(${TEST_NAME} PRIVATE ${WARPDRIVE_TEST_LINK_LIBS})
  endif()
  ...
  if(WARPDRIVE_TEST_MEMCHECK AND NOT ARG_NO_VALGRIND)
    add_test(${TEST_NAME}
             bash
             -c
             "cd '${CMAKE_SOURCE_DIR}'; \
...
# example, if you have src/warpdrive/foo/bar-example.cc, then PREFIX "foo" will
# create test executable foo-bar-example
function(ADD_WARPDRIVE_EXAMPLE REL_EXAMPLE_NAME)
  set(options)
  set(one_value_args)
  set(multi_value_args
      EXTRA_INCLUDES
  ...
  if(EXISTS ${CMAKE_SOURCE_DIR}/examples/warpdrive/${REL_EXAMPLE_NAME}.cc)
    ...
    target_link_libraries(${EXAMPLE_NAME} ${WARPDRIVE_EXAMPLE_LINK_LIBS})
...
function(WARPDRIVE_INSTALL_ALL_HEADERS PATH)
  set(options)
  set(one_value_args)
...
function(WARPDRIVE_ADD_PKG_CONFIG MODULE)
...
function(WARPDRIVE_INSTALL_CMAKE_FIND_MODULE MODULE)
  install(FILES "${WARPDRIVE_SOURCE_DIR}/cmake_modules/Find${MODULE}.cmake"
          DESTINATION "${WARPDRIVE_CMAKE_INSTALL_DIR}")
endfunction()

# Implementations of lisp "car" and "cdr" functions
macro(WARPDRIVE_CAR var)
  set(${var} ${ARGV1})
endmacro()

macro(WARPDRIVE_CDR var rest)
  set(${var} ${ARGN})
endmacro()