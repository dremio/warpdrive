...
static const std::vector<std::pair<int, std::string> > sqltypes {
	...
	std::pair<int, std::string>(0, NULL)};
...
void print_sql_type(int sql_c_type, void *buf, SQLLEN strlen_or_ind, int use_time)
{
	switch (sql_c_type)
	{
	case SQL_C_CHAR:
...
TEST_P(ResultConversionsTest, Basic)
{
	 ...
	 */
	exec_cmd("SET intervalstyle=postgres");						  // TODO: consider removing these
...
INSTANTIATE_TEST_SUITE_P(AllCombinations, ResultConversionsTest,
						 ...
						 [](const testing::TestParamInfo<ResultConversionsTest::ParamType> &info)
						 {
