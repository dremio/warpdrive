...
main(int argc, char **argv)
{
	...
	rc = SQLDriverConnect(conn2, NULL, (SQLCHAR *) buffer, SQL_NTS,
						  NULL, 0, &val,
						  SQL_DRIVER_COMPLETE);
	if (!SQL_SUCCEEDED(rc))
	{
	...
	rc = SQLExecDirect(hstmt,
				...
				SQL_NTS);
	rc = SQLError(env2, conn2, hstmt, (SQLCHAR *) buffer, &valint,
				  (SQLCHAR *) message, 256, &val);
	...
	for (i = 0; i < ARRAY_SIZE; i++)
	{
		int_array[i] = i;
		int_ind_array[i] = 0;
		sprintf((char *) str_array[i], "column num %d", i);
