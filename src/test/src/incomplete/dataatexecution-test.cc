...
int main(int argc, char **argv)
{
	...
	while ((rc = SQLParamData(hstmt, &paramid)) == SQL_NEED_DATA)
	{
	  ...
	  {
		  rc = SQLPutData(hstmt, (SQLCHAR *) "foo", 3);
		  CHECK_STMT_RESULT(rc, "SQLPutData failed", hstmt);
		  /* append more data */
		  rc = SQLPutData(hstmt, (SQLCHAR *) "bar", 3);
		  CHECK_STMT_RESULT(rc, "SQLPutData failed", hstmt);
	  }
	  else
	  {
	...
	{
		if (nprocessed == 1)
			rc = SQLPutData(hstmt, (SQLCHAR *) "foo", strlen("foo"));
		else if (nprocessed == 2)
			rc = SQLPutData(hstmt, (SQLCHAR *) "barf", strlen("barf"));
		else
		{
