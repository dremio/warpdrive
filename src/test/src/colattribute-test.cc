...
TEST_P(SQLColAttributeTest, ColAttributeTestWithExtraConnOptions) {
    ...
    std::vector<long> expected_octet_lengths = {4, 65536, 65536, 65536, 65536};

    std::vector<SQLULEN> expected_data_types_odbc_v2 = {SQL_INTEGER, SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR, SQL_TIMESTAMP, SQL_INTERVAL};
    std::vector<SQLULEN> expected_data_types_odbc_v3 = {SQL_INTEGER, SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR, SQL_TYPE_TIMESTAMP, SQL_INTERVAL};

    std::vector<SQLLEN> expected_concise_types = {SQL_INTEGER, SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR, SQL_TYPE_TIMESTAMP, SQL_INTERVAL_DAY};

    std::vector<SQLULEN> expected_data_types;
    switch (params.m_odbcVersion) {
        case SQL_OV_ODBC2:
            expected_data_types = expected_data_types_odbc_v2;
            break;
        case SQL_OV_ODBC3:
            expected_data_types = expected_data_types_odbc_v3;
            break;
        default:
    ...
    for (i = 1; i <= num_cols; i++) {
        ...
        const SQLULEN expected_data_type = expected_data_types[i - 1];
        ...
        EXPECT_EQ(actual_data_type, expected_data_type) << "Failed at column: " + expected_column_name;
