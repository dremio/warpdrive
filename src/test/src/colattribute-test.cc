...
TEST_P(SQLColAttributeTest, ColAttributeTestWithExtraConnOptions) {
    ...
    for (i = 1; i <= num_cols; i++) {
        char actualColumnName[64];
        char actualTypeName[64];
        SQLLEN actualOctetLength;

        return_code = SQLColAttribute(handle_stmt,
                                      i,
                                      SQL_DESC_LABEL,
                                      actualColumnName,
                                      sizeof(actualColumnName),
                                      nullptr,
                                      nullptr);
        CHECK_STMT_RESULT(return_code, "SQLColAttribute failed", handle_stmt);
        EXPECT_EQ(actualColumnName, expected_names[i - 1]);

        return_code = SQLColAttribute(handle_stmt,
                                      i,
                                      SQL_DESC_TYPE_NAME,
                                      actualTypeName,
                                      sizeof(actualTypeName),
                                      nullptr,
                                      nullptr);
        CHECK_STMT_RESULT(return_code, "SQLColAttribute failed", handle_stmt);
        EXPECT_EQ(actualTypeName, expected_type_names[i - 1]);
        ...
        return_code = SQLColAttribute(handle_stmt,
                                      ...
                                      &actualOctetLength);
        CHECK_STMT_RESULT(return_code, "SQLColAttribute failed", handle_stmt);
        EXPECT_EQ(expected_octet_length[i - 1], actualOctetLength);
    }
}
