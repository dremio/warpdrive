				   ...
				   SQLINTEGER	*pcbValue)
{
  return ODBCConnection::ExecuteWithDiagnostics(hdbc, [&] {
    ...
    MYLOG(0, "Entering\n");
    ret = WD_GetConnectAttr(hdbc, fAttribute, rgbValue, cbValueMax, pcbValue,
                            true);
    return ret;
        });
                                                }
				   ...
				   SQLINTEGER	cbValue)
{
  return ODBCConnection::ExecuteWithDiagnostics(hdbc, [&] {
    ...
    ret = WD_SetConnectAttr(hdbc, fAttribute, rgbValue, cbValue, true);
    return ret;
  });
}
				 ...
				 SQLINTEGER BufferLength)
{
  return ODBCDescriptor::ExecuteWithDiagnostics(DescriptorHandle, [&] {
    ...
    ret = WD_SetDescField(DescriptorHandle, RecNumber, FieldIdentifier, uval ? uval.get() : Value,
...
SQLGetDescFieldW(SQLHDESC hdesc, SQLSMALLINT iRecord, SQLSMALLINT iField,
				 ...
				 SQLINTEGER *pcbValue)
{
  return ODBCDescriptor::ExecuteWithDiagnostics(hdesc, [&] {
    RETCODE ret;
