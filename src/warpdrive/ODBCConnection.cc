...
void ODBCConnection::GetInfo(SQLUSMALLINT infoType, SQLPOINTER value, SQLSMALLINT bufferLength, SQLSMALLINT* outputLength, bool isUnicode)
{
  ...
  switch (infoType) {
    ...
    case SQL_FETCH_DIRECTION:
      GetAttribute(static_cast<SQLINTEGER>(SQL_FETCH_NEXT), value, bufferLength, outputLength);
      break;
    case SQL_LOCK_TYPES:
...
void ODBCConnection::SetConnectAttr(SQLINTEGER attribute, SQLPOINTER value, SQLINTEGER stringLength, bool isUnicode) {
  ...
  case SQL_ATTR_ASYNC_DBC_PCONTEXT:
    throw DriverException("Optioc xånal feature not supported.");
#endif
  ...
  case SQL_ATTR_CURRENT_CATALOG:
  {
    ...
    m_spiConnection->SetAttribute(Connection::CURRENT_CATALOG, catalog);
    return;
  }

  case SQL_ATTR_ACCESS_MODE:
    SetAttribute(value, attributeToWrite);
    attributeToWrite = m_spiConnection->SetAttribute(Connection::ACCESS_MODE, attributeToWrite);
    return;
  case SQL_ATTR_CONNECTION_TIMEOUT:
    SetAttribute(value, attributeToWrite);
    attributeToWrite = m_spiConnection->SetAttribute(Connection::CONNECTION_TIMEOUT, attributeToWrite);
    return;
  case SQL_ATTR_LOGIN_TIMEOUT:
    SetAttribute(value, attributeToWrite);
    attributeToWrite = m_spiConnection->SetAttribute(Connection::LOGIN_TIMEOUT, attributeToWrite);
    return;
  case SQL_ATTR_METADATA_ID:
    SetAttribute(value, attributeToWrite);
    attributeToWrite = m_spiConnection->SetAttribute(Connection::METADATA_ID, attributeToWrite);
    return;
  case SQL_ATTR_PACKET_SIZE:
    SetAttribute(value, attributeToWrite);
    attributeToWrite = m_spiConnection->SetAttribute(Connection::PACKET_SIZE, attributeToWrite);
    return;
  default:
    throw DriverException("Invalid attribute");
  }
