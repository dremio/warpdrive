...
SQLFreeHandle(SQLSMALLINT HandleType, SQLHANDLE Handle)
{
  SQLRETURN rc = SQL_SUCCESS;
	...
	{
		case SQL_HANDLE_ENV:
                  ODBCEnvironment::of(Handle)->ExecuteWithDiagnostics(Handle, rc, [&]() -> SQLRETURN {
                    ret = WD_FreeEnv(Handle);
                                      });
			break;
		case SQL_HANDLE_DBC:
                  ODBCConnection::of(Handle)->ExecuteWithDiagnostics(Handle, rc, [&]() -> SQLRETURN {
                    ret = WD_FreeConnect(Handle);
                                      });
			break;
		case SQL_HANDLE_STMT:
                  ODBCStatement::of(Handle)->ExecuteWithDiagnostics(Handle, rc, [&]() -> SQLRETURN {
                    ret = WD_FreeStmt(Handle, SQL_DROP);
                                      });
			break;
		case SQL_HANDLE_DESC:
                  ODBCDescriptor::of(Handle)->ExecuteWithDiagnostics(Handle, rc, [&]() -> SQLRETURN {
